...
TestIDandFrustration <- dfStat_remove %>%
  group_by(TestID) %>%
...
summary(lm(Frustration ~ TestID, data = dfStat_remove))

# Does the test order of the PAM influence the Control?
TestIDandControl <- dfStat_remove %>%
  group_by(TestID) %>%
...
summary(lm(Control ~ TestID, data = dfStat_remove))

# Does the test order of the PAM influence the Control
TestIDandRank <- dfStat_remove %>%
...
summary(lm(Rank ~ TestID, data = dfStat_remove))

#------- Frustration --------
FrustrationOverview <- dfStat_remove %>%
  group_by(PAM) %>%
  summarise(meanFrustration = mean(Frustration),
            medianFrustration = median(Frustration),
            sdFrustration = sd(Frustration))
...
summary(lm(Frustration ~ PAM, data = dfStat_remove))

#------- Control --------

ControlOverview <- dfStat_remove %>%
  group_by(PAM) %>%
  summarise(meanControl = mean(Control),
            medianControl = median(Control),
            sdControl = sd(Control))
...
summary(lm(Control ~ PAM, data = dfStat_remove))

#------- Rank --------

RankOverview <- dfStat_remove %>%
  dplyr::group_by(PAM) %>%
  summarise(meanRank = mean(Rank),
            medianRank = median(Rank),
            sdRank = sd(Rank))
...
summary(lm(Frustration ~ Control, data = dfStat_remove))

# If we split them up 
summary(lm(Frustration ~ Control * PAM, data = dfStat_remove))
...
# if we look at all the data
summary(lm(Frustration ~ Rank, data = dfStat_remove))

# If we split them up 
summary(lm(Frustration ~ PAM, data = dfStat_remove))

# If we split them up 
summary(lm(Frustration ~ Rank * PAM, data = dfStat_remove))

#------- Frustration vs Gender --------

GenderFrustrationOverview <- dfStat_remove %>%
  group_by(Gender) %>%
...
summary(lm(Frustration ~ Gender, data = dfStat_remove))
...
# Are males and females equal?
summary(lm(Frustration ~ Age, data = dfStat_remove))
...
# if we look at all the data
summary(lm(Control ~ Rank, data = dfStat_remove))

# If we split them up 
summary(lm(Control ~ Rank * PAM, data = dfStat_remove))


#------- Frustration vs Rank ------
dfStat_remove %>% 
  ggplot(aes(x = Rank, y = Frustration)) +
  #geom_violin(shape=16, position=position_jitter(0.1)) +
  #geom_smooth(color = "Black", method = lm, se=F) +
  #geom_jitter()+
  geom_smooth(aes(color = PAM), method = lm, se=F) +
